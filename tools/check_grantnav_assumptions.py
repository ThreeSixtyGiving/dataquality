...
for dataset in data_json:
    print(
        "Checking {}: {} ({})".format(
            dataset["publisher"]["name"], dataset["title"], dataset["identifier"]
        )
    )
    ...
    assert distribution["accessURL"].startswith("http://") or distribution[
        "accessURL"
    ].startswith("https://")
    # Website should start with http:// or https://
    assert dataset["publisher"]["website"].startswith("http://") or dataset[
        "publisher"
    ...
    try:
        with open(
            os.path.join("data/json_all/{}.json".format(dataset["identifier"]))
        ) as fp:
